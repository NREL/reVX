...
from typing import Optional, TypedDict, List, Tuple, Dict
...
# Barrier layers to combine and values to use for the barrier.
# Tuples are in the format:
#
#     ({ras_val1: barrier_val1, ras_val2: barrier_val2, ...}, 'fname.tif')
#
# where ras_valX is the value in the raster, and barrier_valX is the
# desired barrier value. Any values in the raster that are not
# specified in the dict are assumed to not be a barrier. 'fname.tif'
# is the file name of the raster.
BarrierDefinition = Tuple[Dict[int, float], str]
...
class OffshoreCostCreator(CombineRasters):
    ...
    # flake8: noqa: C901
    def build_off_shore_barriers(self,
                                 barrier_files: List[BarrierDefinition],
                                 slope_file: Optional[str] = None,
                                 slope_barrier_cutoff: float = 15,
                                 slope_barrier_value: float = 1,
                                 bathy_file: Optional[str] = None,
                                 bathy_depth_cutoff: float = -1000,
                                 bathy_barrier_value: float = 1,
                                 save_tiff = False):
        """
        Combine offshore friction layers. TODO
        ...
        bathy_depth_cutoff : float, optional
            Depth below which a barrier is applied. This must in the same
            units as the bathy file.
        bathy_barrier_value : int, optional
            Friction value to apply to areas with a depth great than
            bath_depth_cutoff.
        save_tiff : bool, optional
            Save composite friction to tiff if true
        """
        logger.info('Processing barrier layers')
        b_layers = {}

        # Add bathymetry to barrier dict
        if bathy_file is not None:
            logger.info('--- calculating bathymetric barriers')
            logger.debug('--- --- bathy_depth_cutoff is %s',
                         bathy_depth_cutoff)
            logger.debug('--- --- bathy_barrier_value is %s',
                         bathy_barrier_value)
            ...
            with rio.open(bathy_file) as f:
                d = f.read(1)
            assert d.shape == self._os_shape
            d2 = np.where(d >= bathy_depth_cutoff, 0, bathy_barrier_value)

            b_layers[bathy_file] = d2.astype('uint16')
        ...
        if slope_file is not None:
            ...
            d2 = np.where(d < self._low_slope_cutoff, self._low_slope_friction,
                          d2)
            b_layers[slope_file] = d2.astype('uint16')
        ...
        for fr_dict, f in friction_files:
            ...
            assert d.shape == self._os_shape and d.min() == 0
            b_layers[f] = d.astype('uint16')

        logger.info('--- combining all offshore friction layers')
        self._os_friction = reduce(_sum, b_layers.values()).astype('uint16')
