...
class FrictionBarrierBuilder:
    ...
    def _process_forced_inclusions(self, data: npt.NDArray,
        ...
        for fname, config in fi_layers.items():
            if Path(fname).suffix.lower() not in ['.tif', '.tiff']:
                logger.error(f'Forced inclusion file {fname} does not end '
                             'with .tif. GeoTIFFs are the only format '
                             'allowed for forced inclusions.')
            ...
            if map_given or range_given or rasterize_given:
                logger.error('`map`, `range`, and `rasterize` are not '
                             'allowed if `forced_inclusion` is True, but '
                             f'one was found in config: {fname}: {config}')
    ...
    def __check_tiff_layer_config(config: FBLayerConfig):
        ...
        if config.map is not None and config.range is not None:
            raise ValueError('Keys "map" and "range" are exclusionary but '
