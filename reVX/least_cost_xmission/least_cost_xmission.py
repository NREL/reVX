...
class LeastCostXmission(LeastCostPaths):
    ...
    def __init__(
        ...
        (self._sc_points, self._features, self._sub_lines_mapping, self._shape) = (
            self._map_to_costs(
                cost_fpath,
                features_fpath,
                resolution=resolution,
    ...
    def __repr__(self):
        msg = "{} to be computed for {} sc_points and {} features".format(
            self.__class__.__name__, len(self.sc_points), len(self.features)
        )
    ...
    def _check_layers(self):
        ...
        self._check_layers_in_h5(
            self._cost_fpath, [self._tb_layer_name, self._iso_layer_name]
        )
    ...
    def _create_sc_points(cost_fpath, resolution=RESOLUTION):
        ...
        sc_points : gpd.GeoDataFrame
            SC points
        """
        logger.debug("Loading Supply Curve Points for res: {}".format(resolution))
        sce = SupplyCurveExtent(cost_fpath, resolution=resolution)
        sc_points = sce.points.rename(
            columns={"row_ind": "sc_row_ind", "col_ind": "sc_col_ind"}
        )
    ...
    def _get_feature_cost_indices(features, crs, transform, shape):
        ...
        """
        row, col, mask = super(
            LeastCostXmission, LeastCostXmission
        )._get_feature_cost_indices(features, crs, transform, shape)

        t_lines = features["category"] == TRANS_LINE_CAT
        mask |= t_lines

        row[t_lines] = np.where(row[t_lines] >= 0, row[t_lines], 0)
        row[t_lines] = np.where(row[t_lines] < shape[0], row[t_lines], shape[0] - 1)
        col[t_lines] = np.where(col[t_lines] >= 0, col[t_lines], 0)
        col[t_lines] = np.where(col[t_lines] < shape[1], col[t_lines], shape[1] - 1)
    ...
    def _map_to_costs(
        ...
        row, col, mask = cls._get_feature_cost_indices(features, crs, transform, shape)
        if any(~mask):
            msg = (
        ...
        x, y = rasterio.transform.xy(
            transform, sc_points["row"].values, sc_points["col"].values
        )
    ...
    def _clip_to_sc_point(
        ...
        if len(self.sink_coords) > 2 or radius:
            ...
            if radius is None:
                ...
                logger.debug(
                    "Radius to {} nearest sink is: {}".format(nn_sinks, radius)
                )
            else:
            ...
            sc_features = self._clip_to_radius(
                sc_point,
                radius,
                sc_features,
                clipping_buffer,
        ...
        logger.debug(
            "{} transmission features found in clipped area with "
            "radius {} and minimum max voltage of {}".format(
                len(sc_features), radius, tie_line_voltage
            )
        )
        ...
        if mask.any():
            ...
            trans_gids = np.concatenate(self.sub_lines_mapping.loc[trans_gids].values)
            ...
            logger.debug(
                "Adding all {} transmission lines connected to "
                "substations with minimum max voltage of {}".format(
                    len(trans_lines), tie_line_voltage
                )
            )
    ...
    def _clip_to_radius(
        ...
        logger.info(
            "{} transmission features found in clipped area with radius {}".format(
                len(clipped_sc_features), radius_m
            )
        )
        return clipped_sc_features.copy(deep=True)

    def process_sc_points(
        self,
        capacity_class,
        cost_layers,
        sc_point_gids=None,
        ...
        if max_workers > 1:
            logger.info(
                "Computing Least Cost Transmission for %d SC points "
            ...
            logger.info(
                "Computing Least Cost Transmission for {:,} SC points in serial".format(
                    len(sc_point_gids)
                )
            )
        ...
        least_costs = pd.concat(least_costs).sort_values(["sc_point_gid", "trans_gid"])
        capacity_class = self._config._parse_cap_class(capacity_class)
        least_costs["max_cap"] = self._config["power_classes"][capacity_class]
        lcp_frac = len(least_costs["sc_point_gid"].unique()) / len(sc_point_gids) * 100
        logger.info(
            "{:.4f}% of requested sc point gids were successfully "
    ...
    def _compute_paths_in_chunks(
        ...
        if (costs < 0).any():
            ...
            msg = (
                "Could not connect SC points with invalid starting costs "
    ...
    def _process_single_core(
        ...
        for _, sc_point in self.sc_points.iterrows():
            ...
            if gid in sc_point_gids:
                ...
                count += 1
                logger.info(
                    "SC point {} of {} complete!".format(count, len(sc_point_gids))
                )
                log_mem(logger)
        return least_costs
    ...
    def run(
        ...
        logger.info(
            "{} connections were made to {} SC points in {:.4f} minutes".format(
                len(least_costs),
...
class RegionalXmission(LeastCostXmission):
    ...
    def _clip_to_sc_point(
        ...
        **___,
        ...
        logger.debug(
            "%d transmission features found in clipped area ", len(sc_features)
        )
        ...
        radius = _compute_radius(sc_features, sc_point, transform, clipping_buffer)
        logger.debug(
            "%d transmission features found in clipped area of radius %.2f",
            len(sc_features),
            radius,
        )
