...
Results of each step of the process will be written to ``out_dir``. Note that additional options are available for each step of the process: ``classify``, ``profiles``, and ``timeslices``.
...
NOTE: when running profiles without classify the ReEDS classification table must be supplied.
...
NOTE: when running timeslices without profiles the ReEDS representative profiles must be supplied.
